...
<head>
    ...
    <title>{{ $center['name'] }} - Klinik Koehried</title>
    ...
    <section class="pt-24 pb-4 bg-gradient-to-b from-brand-50/50 to-white">
        <div class="mx-auto max-w-7xl px-6">
            <nav class="text-sm text-ink-500 flex items-center gap-2">
                ...
                <a href="{{ route('unggulan.index') }}" class="hover:text-brand-600">Pusat Unggulan</a>
                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                <span class="text-ink-800 font-medium">{{ $center['name'] }}</span>
            </nav>
        </div>
    </section>
        ...
        <div class="mx-auto max-w-7xl px-6">
            <div class="glass rounded-3xl p-8 sm:p-12 shadow-xl border border-white/50" style="background: linear-gradient(135deg, rgba(37,99,235,0.04) 0%, rgba(255,255,255,0.9) 100%);">
                <div class="flex flex-col md:flex-row items-start md:items-center gap-6">
                    <div class="h-20 w-20 rounded-2xl {{ $center['iconBg'] }} flex items-center justify-center flex-shrink-0">
                        <svg class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $center['icon'] }}" /></svg>
                    </div>
                    <div class="flex-1">
                        <h1 class="font-display text-3xl sm:text-4xl font-extrabold text-ink-900">{{ $center['name'] }}</h1>
                        <p class="text-ink-600 mt-3 text-base leading-relaxed max-w-3xl">{{ $center['description'] }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="pb-28">
        <div class="mx-auto max-w-7xl px-6">
            <div class="grid lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 space-y-8">
                    <div class="detail-card sr">
                        <div class="flex items-center gap-3 mb-5">
                            <div class="h-8 w-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg></div>
                            <h2 class="font-display font-bold text-xl text-ink-900">Keunggulan Layanan</h2>
                        </div>
                        <ul class="grid sm:grid-cols-2 gap-3">
                            @foreach($center['unggulan'] as $u)
                    ...
                    <div class="detail-card sr">
                        <div class="flex items-center gap-3 mb-5">
                            ...
                            <h2 class="font-display font-bold text-xl text-ink-900">Teknologi yang Digunakan</h2>
                        </div>
                        <ul class="grid sm:grid-cols-2 gap-3">
                            @foreach($center['teknologi'] as $t)
                            <li class="flex items-start gap-3 text-sm text-ink-600">
                    ...
                    <div class="detail-card sr">
                        <div class="flex items-center gap-3 mb-5">
                            <div class="h-8 w-8 rounded-lg bg-teal-50 text-teal-600 flex items-center justify-center"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg></div>
                            <h2 class="font-display font-bold text-xl text-ink-900">Jenis Penanganan</h2>
                        </div>
                        <ul class="grid sm:grid-cols-2 gap-3">
                            @foreach($center['penanganan'] as $pn)
                            <li class="flex items-start gap-3 text-sm text-ink-600">
                                <svg class="h-5 w-5 text-teal-500 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4" /></svg>
                                {{ $pn }}
                            </li>
                            @endforeach
                        </ul>
                    ...
                    @if(count($center['doctors']) > 0)
                    <div class="detail-card sr">
                        <div class="flex items-center gap-3 mb-5">
                            <div class="h-8 w-8 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div>
                            <h2 class="font-display font-bold text-xl text-ink-900">Tim Dokter Spesialis</h2>
                        </div>
                        <div class="grid sm:grid-cols-2 gap-4">
                            @foreach($center['doctors'] as $doc)
                ...
                <div class="space-y-6">
                    <div class="detail-card sr">
                        <h3 class="font-display font-bold text-lg text-ink-900 mb-4">Informasi Pusat</h3>
                        <div class="space-y-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-ink-800 font-medium mt-0.5">{{ $center['hours'] }}</p>
                                </div>
                            </div>
                            <div class="border-t border-ink-100 pt-4 flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs font-bold text-ink-500 uppercase tracking-wider">Lokasi</p>
                                    <p class="text-sm text-ink-800 font-medium mt-0.5">Jalan Kesehatan No. 123, Jakarta Selatan</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="rounded-2xl p-6 text-white sr" style="background: linear-gradient(135deg, #2563eb 0%, #1e40af 100%);">
                        <h3 class="font-display font-bold text-lg">Konsultasi Sekarang</h3>
                        <p class="text-sm text-brand-200 mt-2 leading-relaxed">Dapatkan penanganan dari tim spesialis kami. Hubungi untuk informasi lebih lanjut.</p>
                        <div class="mt-5 flex flex-col gap-3">
                            ...
                            <a href="{{ route('kontak') }}" class="flex items-center justify-center gap-2 rounded-full bg-white text-brand-700 hover:bg-slate-100 px-5 py-3 text-sm font-bold transition-all">Hubungi Kami</a>
                        </div>
                    </div>

                    <div class="detail-card sr">
                        <h3 class="font-display font-bold text-sm text-ink-900 mb-3">Pusat Unggulan Lainnya</h3>
                        <div class="space-y-1">
                            @foreach($centers as $cSlug => $c)
                            @if($cSlug !== $slug)
                            <a href="{{ route('unggulan.show', $cSlug) }}" class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-brand-50 text-sm text-ink-600 hover:text-brand-700 transition-all">
                                <div class="h-6 w-6 rounded-md {{ $c['iconBg'] }} flex items-center justify-center flex-shrink-0">
                                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $c['icon'] }}" /></svg>
                                </div>
                                {{ $c['name'] }}
                            </a>
                            @endif
